(function () {
  ...
  function initTryOnWidget(root) {
    ...
    const proxyPath = root.dataset.appProxyPath || "/apps/tryon";
    ...
    async function handleShare() {
      ...
      if (navigator.share) {
        await navigator.share({ title: "My AI Try On", text: `Try-on result for ${productTitle}.`, url });
        return;
      }
    ...
    function updatePrimaryState() {
      els.primary.disabled = state.step === "processing" || (state.step === "upload" && !state.uploadData);
      const labels = { intro: "Start Try On", upload: state.uploadData ? "Generate Look" : "Upload Photo", processing: "Generating...", result: "Add to Cart" };
